...
def plotstuff(ALL_SPS,GOOD_SPS,WF_CANDS):
    ...
    if is_all:
        ...
        if np.dim(all_dat)==1: all_dat = np.zeros((1,5))
        all_DM = all_dat[:,0]
        all_SN = all_dat[:,1]
        all_T = all_dat[:,2]
    if is_good:
        good_dat = np.loadtxt(GOOD_SPS)
        if np.dim(good_dat)==1: good_dat = np.zeros((1,5))
        good_DM = [:,0]
        good_SN = [:,1]
        good_T = [:,2]
    if is_wf:
        wf_dat = np.loadtxt(WF_CANDS)
        if np.dim(wf_dat)==1: wf_dat = np.zeros((1,5))
        wf_DM = [:,0]
        wf_SN = [:,1]
        wf_T = [:,2]
    ...
    fig.subplot_adjust(wspace=.7,hspace=.7)
